...
def getZalo(database_path, vocal_path, public_test=True):
    ...
    zalo_audio_path = os.path.join(database_path, 'songs')
    ...
    for zalo_anno in tqdm(zalo_annots):
      if zalo_anno.split('/')[-1] in ["37393436375f38.json", "label_gt_txt", "txt_train.zip", ".ipynb_checkpoints"]:
        continue
...
def get_zalo_folds(database_path, vocal_path, public_test=True):
    ...
    # return {"train" : dataset[train_idxs], "val" : dataset[val_idxs]}
...
# alphabet = "abcdeghiklmnopqrstuvxyz '~"

# def txt2seq_vn(s):
#   idxs = []
#   for c in s:
#     try:
#       idxs.append(alphabet.index(c))
#     except Exception as e:
#       idxs.append(alphabet.index(" "))
