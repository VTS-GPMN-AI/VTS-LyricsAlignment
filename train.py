...
from wrapper import preprocess_lyrics
...
def main(args):
    ...
    if os.path.exists(os.path.join(args.hdf_dir, "val.hdf5")) and os.path.exists(os.path.join(args.hdf_dir, "train.hdf5")):
        ...
        zalo_datasets = get_zalo_folds("/content/drive/MyDrive/Zalo_AI/dataset/train", "/content/vocals/", public_test=True)
    
    val_data = LyricsAlignDataset(zalo_datasets, "val", args.sr, hparams['input_sample'], args.hdf_dir, dummy=args.dummy)
    train_data = LyricsAlignDataset(zalo_datasets, "train", args.sr, hparams['input_sample'], args.hdf_dir, dummy=args.dummy)
