...
def iou_zalo(preds, gts):
  num_word = 1e-5
  iou_overall = 0
  for pred, gt in zip(preds, gts):
      num_word += 1
      intersect = max(0, abs(min(pred[1], gt[1])-max(pred[0], gt[0])))
      union = (max(pred[1], gt[1])-min(pred[0], gt[0]))
      iou = intersect/union
      iou_overall += iou
  if num_word == 0:
    return 0
  return min(1 - iou_overall/num_word, 1)
...
def write_csv(pred_file, word_align, words):
    ...
    with open(pred_file, 'w') as f:
        for j in range(len(word_align)):
            ...
            f.write("{},{},{}\n".format(word_time[0] * resolution, word_time[1] * resolution, words[j].replace(",", '').replace('"', "")))
